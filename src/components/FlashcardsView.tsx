...
export const FlashcardsView: React.FC = () => {
  ...
  const handleTranslateCustom = async (e: React.FormEvent) => {
    ...
    try {
      const data = await translatePhrase(customPhrase);
      if (data && data.hanzi) {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-1 sm:px-4 py-3">
      {/* Header & Instructions */}
      <motion.div 
      ...
      >
        ...
        <form onSubmit={handleTranslateCustom} className="pt-1">
          <div className="bg-slate-950 p-3 rounded-xl border border-slate-800/90 flex flex-col sm:flex-row items-center gap-2.5 shadow-inner">
            ...
            <input
              ...
              className="flex-1 bg-slate-900 border border-slate-700/80 rounded-lg px-3.5 py-2 text-xs sm:text-sm text-white focus:outline-none focus:border-purple-500 w-full transition-colors"
            />
            <button
              type="submit"
              disabled={isTranslating || !customPhrase.trim()}
              className="w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 disabled:opacity-50 text-white rounded-lg text-xs font-semibold flex items-center justify-center space-x-1.5 transition cursor-pointer shadow-md shadow-purple-950/40 active:scale-95"
            >
        ...
        <AnimatePresence>
          {translatedResult && (
            <motion.div
              ...
              className="p-4 bg-purple-950/40 border border-purple-500/40 rounded-xl space-y-2 relative"
            >
              <button
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <div className="flex space-x-2">
                  <button
                    ...
                    className="p-2 rounded-lg bg-slate-800 text-slate-200 hover:text-white flex items-center gap-1 text-xs active:scale-95 transition-transform"
                  >
                  ...
                  <button
                    ...
                    className="p-2 rounded-lg bg-purple-600 text-white flex items-center gap-1 text-xs font-semibold active:scale-95 transition-transform"
                  >
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition-colors cursor-pointer active:scale-95 select-none ${
                  isSelected
                    ? 'text-white'
              ...
              >
                {isSelected && (
                  <motion.div
                    layoutId="flashcardCatPill"
                    className="absolute inset-0 bg-gradient-to-r from-red-600 to-red-700 rounded-xl shadow-md border border-red-500/40"
        ...
        <div className="w-full sm:w-64">
          <input
            type="text"
            placeholder="Satz suchen (z. B. Toilette, Rechnung)..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full bg-slate-900/90 border border-slate-800 rounded-xl px-3.5 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-red-500 transition-colors"
          />
        </div>
      </div>
          ...
          {filteredCards.map((card) => {
            ...
            return (
              <motion.div
                ...
                key={card.id}
                className={`bg-slate-900/90 rounded-2xl border transition-all duration-200 p-4 sm:p-5 flex flex-col justify-between space-y-3.5 shadow-lg ${
                  isEmergency
              ...
              >
                <div className="space-y-3">
                  ...
                  <div className="bg-slate-950/80 p-3.5 rounded-xl border border-slate-800/80 text-center shadow-inner">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-800/80">
                  <button
                    ...
                    className={`p-2 rounded-lg transition cursor-pointer flex items-center space-x-1 text-xs font-semibold active:scale-95 ${
                  ...
                  <div className="flex space-x-1.5">
                    <button
                      ...
                      className="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition cursor-pointer text-xs active:scale-95"
                    ...
                    <button
                      ...
                      className="px-3 py-2 rounded-lg bg-red-600/20 hover:bg-red-600/30 text-red-300 border border-red-500/30 text-xs font-semibold flex items-center space-x-1 transition cursor-pointer active:scale-95"
        ...
        {fullscreenCard && (
          ...
          >
            <button
              ...
              className="absolute top-6 right-6 p-3 rounded-full bg-slate-800/90 text-white hover:bg-slate-700 cursor-pointer shadow-lg active:scale-90 transition-transform"
            >
              <X className="w-6 h-6" />
            </button>
            ...
            <motion.div 
              ...
              className="max-w-2xl w-full space-y-6 bg-slate-900/95 p-8 sm:p-12 rounded-3xl border border-slate-700 shadow-2xl shadow-black"
            >
              ...
              <div className="py-4">
                <h1 className="text-4xl sm:text-6xl font-black text-white tracking-wide leading-tight drop-shadow-md">
              ...
              <div className="flex justify-center space-x-3 pt-4">
                <button
                  ...
                  className="px-5 py-3 rounded-xl bg-slate-800 hover:bg-slate-700 text-white font-semibold flex items-center space-x-2 transition cursor-pointer active:scale-95 shadow-md"
                >
                ...
                <button
                  onClick={() => setFullscreenCard(null)}
                  className="px-5 py-3 rounded-xl bg-gradient-to-r from-red-600 to-rose-600 hover:from-red-500 hover:to-rose-500 text-white font-semibold transition cursor-pointer active:scale-95 shadow-lg shadow-red-950/50"
                >
                  Schließen
                </button>
              </div>
