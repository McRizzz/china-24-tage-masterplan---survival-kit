          ...
          {SURVIVAL_GUIDES.map((section) => {
            ...
            return (
              <button
                ...
                className={`relative p-2 rounded-xl text-left border transition-colors cursor-pointer flex-shrink-0 flex items-center space-x-2 snap-start select-none active:scale-95 ${
                  isSelected
              ...
              >
                {isSelected && (
                  <motion.div
                    layoutId="survivalTopicPill"
                    className="absolute inset-0 bg-slate-800 rounded-xl shadow-md border border-red-500/60"
                    transition={{ type: 'spring', stiffness: 450, damping: 32 }}
                  />
                )}
                <div className={`relative z-10 p-1 rounded-md ${isSelected ? 'text-red-400' : 'text-slate-500'}`}>
                  {getIcon(section.iconName)}
                </div>
                <span className="relative z-10 text-xs font-semibold whitespace-nowrap">{section.title}</span>
              </button>
            );
          })}
        </div>
          ...
          {(() => {
            ...
            return (
              <motion.div
                ...
                className="bg-slate-900 rounded-2xl border border-slate-800 p-4 space-y-4 shadow-xl"
              >
                {/* Header */}
