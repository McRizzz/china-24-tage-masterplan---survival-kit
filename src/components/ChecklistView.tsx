...
export const ChecklistView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto px-1 sm:px-4 py-3">
      ...
      >
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center space-x-3">
            <button
              onClick={resetAll}
              className="p-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white transition cursor-pointer text-xs font-semibold flex items-center gap-1.5 active:scale-95 shadow-md"
        ...
        {/* Progress Bar */}
        <div className="bg-slate-950/80 p-3.5 rounded-xl border border-slate-800 space-y-2 shadow-inner">
      ...
      <form onSubmit={handleAddCustom} className="bg-slate-900/90 p-3.5 rounded-2xl border border-slate-800 flex flex-col sm:flex-row items-center gap-2.5 shadow-md">
        <div className="flex items-center space-x-2 text-slate-400 text-xs font-semibold flex-shrink-0">
          <Plus className="w-4 h-4 text-emerald-400" />
          <span>Eigenen Eintrag:</span>
        </div>
        <input
          type="text"
          placeholder="z. B. Bequeme Laufschuhe, Reiseadapter..."
          value={newTitle}
          onChange={(e) => setNewTitle(e.target.value)}
          className="flex-1 bg-slate-950 border border-slate-700/80 rounded-xl px-3.5 py-2 text-xs text-white focus:outline-none focus:border-emerald-500 w-full transition-colors"
        />
        <select
          value={newCategory}
        ...
        <button
          ...
          className="w-full sm:w-auto px-4 py-2 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 disabled:opacity-50 text-white rounded-xl text-xs font-bold transition cursor-pointer active:scale-95 shadow-md shadow-emerald-950/40"
        >
          Hinzufügen
        </button>
      </form>
      ...
      <div className="space-y-4">
        {categories.map((cat) => {
          ...
          return (
            <div key={cat.id} className="bg-slate-900/90 rounded-2xl border border-slate-800 p-4 sm:p-5 space-y-3 shadow-lg">
              <div className="flex items-center space-x-2 pb-2 border-b border-slate-800/80">
                <Icon className="w-4 h-4 text-red-400" />
                <h3 className="text-sm sm:text-base font-bold text-white">{cat.label}</h3>
              </div>
                ...
                {itemsInCat.map((item) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`p-3.5 rounded-xl border transition-all flex items-start justify-between gap-3 cursor-pointer select-none active:scale-[0.99] ${
                        isChecked
                          ? 'bg-emerald-950/20 border-emerald-500/30'
                          : 'bg-slate-950/70 border-slate-800/90 hover:bg-slate-800/40'
                      }`}
                    >
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <div className="flex items-center space-x-2 flex-wrap">
                            <span className={`text-xs sm:text-sm font-semibold text-white transition-colors ${isChecked ? 'line-through text-slate-400' : ''}`}>
                              {item.title}
                            </span>
                            {item.essential && (
                          ...
                          <p className={`text-[11px] text-slate-400 mt-0.5 leading-relaxed ${isChecked ? 'line-through text-slate-500' : ''}`}>
                            {item.detail}
                          </p>
                        </div>
                      </div>
                      ...
                      {isCustom && (
                        <button
                          ...
                          className="text-slate-500 hover:text-red-400 p-1 transition cursor-pointer active:scale-90"
                        >
