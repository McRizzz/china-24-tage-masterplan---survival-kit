      ...
      <div className="space-y-2">
        ...
        <div className="flex items-center space-x-1.5 overflow-x-auto pb-1 scrollbar-none">
          <button
            ...
            className={`relative px-3 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition cursor-pointer select-none ${
              selectedStation === 'all'
                ? 'text-white'
          ...
          >
            {selectedStation === 'all' && (
              <motion.div
                layoutId="stationFilterPill"
                className="absolute inset-0 bg-red-600 rounded-xl shadow-md border border-red-500/50"
                transition={{ type: 'spring', stiffness: 500, damping: 35 }}
              />
            )}
            <span className="relative z-10">Alle ({ITINERARY_DAYS.length})</span>
          </button>
          {STATIONS.map((st) => {
            ...
            return (
              <button
                ...
                className={`relative px-2.5 py-1.5 rounded-xl text-xs font-semibold whitespace-nowrap transition cursor-pointer select-none ${
                  isSel
                    ? 'text-white'
              ...
              >
                {isSel && (
                  <motion.div
                    layoutId="stationFilterPill"
                    className="absolute inset-0 bg-slate-800 border border-red-500/80 rounded-xl shadow-md"
                    transition={{ type: 'spring', stiffness: 500, damping: 35 }}
                  />
                )}
